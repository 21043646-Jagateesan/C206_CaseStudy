...
//	private static String rID = "";
	...
	public static void main(String[] args) {
		...
		QuotationMain mw = new QuotationMain();
	...
	public void start() {
		...
		while (option != 4) {
			...
			if (option == OPTION_VIEW) {
				...
				QuotationMain.viewAllQuotation(quotationList);
				
				
			} else if (option == OPTION_ADD) {
				Quotations qt = inputQuotation();
				QuotationMain.addQuotation(quotationList, qt);
				
			} else if (option == OPTION_DELETE) {
				String Qid = Helper.readString("\nEnter quotation id > ");
				QuotationMain.deleteQuotation(quotationList, Qid);
	...
	private static void viewAllQuotation(ArrayList<Quotations> quotationList1) {
		if (quotationList.size() > 0) {
			QuotationMain.setHeader("QuotationList LIST");
	...
	private static void menu() {
		QuotationMain.setHeader("Designer Interface");
	...
	public static void deleteQuotation(ArrayList<Quotations> quotationList, String Qid) {
		...
		if (QuotationMain.testExists(quotationList, Qid)) {
